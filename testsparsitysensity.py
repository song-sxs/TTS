...
def wanda_plus_plus_pruning(model, input_sets, input_sets_trans, alpha, K, sparsity,target_block_name):
    """
    Args:
...
def create_audio_tuples(dataset_path, output_size):
    """
...
def split_audio_tuples(dataset_path, n_prune, n_eval):
    """
    随机分割数据集，返回不重叠的prune_pairs和eval_pairs
    """
    # 先收集所有可用的(text, context, audio_path)元组
    all_pairs = []
    id2files = defaultdict(list)
    for root, _, files in os.walk(dataset_path):
        for f in files:
    ...
    for id_key, file_list in id2files.items():
        ...
        for text_path in file_list[1:]:
            with open(text_path + '.normalized.txt', 'r', encoding='utf-8') as f:
                text_str = f.read().strip()
            all_pairs.append((text_str, context_str, prompt_speech))
    # 随机打乱
    random.shuffle(all_pairs)
    # 分割
    pairs = all_pairs[:n_prune]
    eval_pairs = all_pairs[n_prune:n_prune+n_eval]
...
def build_block_input_sets(model, pairs):
    """
    根据输入pairs，生成block_input_sets_res和block_input_sets
    """
    ...
    return block_input_sets_res, block_input_sets
...
if __name__ == "__main__":
    ...
    # eval_pairs = create_audio_tuples(train_clean_100_path, 10) 
    pairs, eval_pairs = split_audio_tuples(train_clean_100_path, 128, 10)

    block_input_sets_res, block_input_sets = build_block_input_sets(model, pairs)  

    all_blocks = CAUSAL_RESNET_BLOCKS + BASIC_TRANSFORMER_BLOCKS
    sparsity_ls = [0.5, 0.75]
    results = []
    ...
    for block_name in all_blocks:
        for sparsity in sparsity_ls:
            try:
                ...
                pruned_model = wanda_plus_plus_pruning(flow_blocks, block_input_sets_res, block_input_sets, 100, 5, sparsity, block_name)
                cosyvoice.model.flow = pruned_model

                score_list = []
                for k in range(len(eval_pairs)):
                    prompt_speech_16k = load_wav(eval_pairs[k][2], 16000)
                    for i, j in enumerate(cosyvoice.inference_zero_shot(eval_pairs[k][0], eval_pairs[k][1], prompt_speech_16k, stream=False)):
                ...
                if block_name in CAUSAL_RESNET_BLOCKS:
                    block_type_name = "res" + block_name
                elif block_name in BASIC_TRANSFORMER_BLOCKS:
                    block_type_name = "trans" + block_name
                else:
                    block_type_name = block_name
                results.append([block_type_name] + column_means_rounded.tolist())
            except Exception as e:
                print(f"Error in block {block_name} with sparsity {sparsity}: {e}")
                results.append([block_name, "error", "error", "error", "error"])
                continue
            
    # 保存结果
    df = pd.DataFrame(results, columns=["block_name", "nmos", "mos_sig", "mos_bak", "mos_ovr"])
    df.to_csv("blockwise_scores.csv", index=False)
    print(f"Results saved to blockwise_scores.csv with {len(results)} blocks")
